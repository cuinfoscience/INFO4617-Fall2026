    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.35\textwidth}
            ...
            \begin{alertblock}{Bring a laptop}
                \footnotesize Wednesday is hands-on --- we watch the requests, and the packets, fly.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{The lower stack: client--server, TCP/IP \& DNS}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \textbf{TCP/IP --- the transport.} Every machine on a public network has an IP address. IPv4 offers $2^{32}\approx4.3$ billion of them; IPv6 offers $2^{128}\approx3.4\times10^{38}$. Data hops router to router to arrive --- as discrete \textbf{packets}, each wrapped in its own routing headers.
...
\begin{frame}{Inspecting a page: the Inspector tab}
    ...
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{itemize}\small
                \item What you see is the \textbf{DOM}, not the original HTML --- the page \emph{after} JavaScript finished modifying it. View Source shows the original bytes; if an element is in the Inspector but missing from \texttt{requests.get(url).text}, JavaScript put it there.
                \item \textbf{Hover} a node to highlight it on the page; the \textbf{element picker} icon does the reverse --- click the page, land on the node.
            \end{itemize}
        \end{column}
    ...
    \medskip
    \small Start on a plain Wikipedia article --- commercial sites bury structure under tracking scripts.
\end{frame}

\begin{frame}{Inspecting requests: the Network tab}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            Same DevTools window, \textbf{Network} tab. Reload \emph{after} opening it --- it only records what happens while visible. Check \textbf{Preserve log} to survive a redirect or multi-step flow.

            \bigskip

            A page load is dozens or hundreds of requests: HTML, CSS, JS, images, trackers. The \textbf{filter bar} narrows it --- \textbf{Fetch/XHR} is where API calls and dynamically-loaded data live. Each row shows Name, Status, Type, Size, and a \textbf{Waterfall} (queuing, DNS, connect, TTFB, download).

            \bigskip

            Click a request for \textbf{Headers} (incl.\ \texttt{User-Agent}, \texttt{Cookie}), \textbf{Response} (the raw JSON), and \textbf{Timing}. Right-click \textbf{Copy} $\to$ \textbf{Copy as cURL} reproduces the exact request --- headers, cookies, body.
        \end{column}
        \begin{column}{0.35\textwidth}
            \centering
...
\begin{frame}{Domain names aren't all \texttt{.com}}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            ...
            The U.S.\ has its own country-code TLD, \texttt{.us} --- and almost nobody uses it: roughly three-quarters of U.S.\ sites use \texttt{.com} instead. Most other countries went the opposite way: in Germany, \texttt{.de} is the \emph{default}, unmarked choice for a German site, not a special-purpose alternative the way \texttt{.us} is at home.
        \end{column}
        \begin{column}{0.4\textwidth}
            \begin{block}{Two wrinkles to recognize}
                \footnotesize
                \textbf{Repurposed ccTLDs.} \texttt{.io} (British Indian Ocean Territory $\to$ ``input/output''), \texttt{.ai} (Anguilla $\to$ AI startups), \texttt{.tv} (Tuvalu $\to$ streaming). Ordinary ccTLDs underneath --- you cannot infer origin from the letters.

                \medskip

                \textbf{Non-Latin domains.} DNS is ASCII-only; a Cyrillic domain shown in your address bar is transparently \textbf{punycode} underneath: \texttt{xn--80aswg.xn--p1ai}. An \texttt{xn--} prefix in \texttt{requests} or \texttt{urlparse()} output is this encoding, not an error.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{The upper stack: HTTP status codes \& URLs}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \end{itemize}
            \footnotesize The Network tab's \textbf{Status} column already shows you these, before you open a notebook.
        \end{column}
        \begin{column}{0.35\textwidth}
...
\begin{frame}{User-Agent spoofing: identity is self-reported}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            Every request so far has identified \emph{honestly} --- \texttt{WebDataScience/1.0 (you@colorado.edu)}. \textbf{Spoofing} means sending a \texttt{User-Agent} that claims to be a browser or device you are not.

            \bigskip

            The protocols in this chapter run on an \textbf{honor system}: a client just states who it is. Legitimate reasons to change it exist (testing your own site's mobile layout); so do illegitimate ones.

            \bigskip

            \footnotesize A changed \texttt{User-Agent} string alone is weak and easily noticed --- not a reliable way to look like a real browser, and modern \textbf{Client Hints} headers can expose the mismatch.
        \end{column}
        \begin{column}{0.4\textwidth}
            \begin{alertblock}{Ethical \& legal stakes}
                \footnotesize Not illegal as a technical act --- what matters is \emph{what you do it for}. Checking your own site's rendering is unremarkable; defeating a site's stated block is a different act, with the same CFAA/ToS stakes Ch.~2 covers.

                \smallskip

                Class rule: identify honestly by default; treat a block as a signal to stop, not to route around. Ask your instructor first if a research need suggests otherwise.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}
...
\end{lstlisting}
    \footnotesize Before reaching for Python, \texttt{ping en.wikipedia.org} from any terminal is the fastest reachability check --- no install required, works on every platform.
\end{frame}
