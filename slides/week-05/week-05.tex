...
\begin{frame}{A brief history: ARPANET and a persistent myth}
    \begin{columns}[T]
        \begin{column}{0.55\textwidth}
            \begin{itemize}\small
                \item Paul Baran (RAND, early 1960s) proposed \textbf{packet-switching} specifically so communications could survive a nuclear attack --- no single point of failure to knock out.
                \item \textbf{ARPANET} (1969, DARPA) used packet-switching, but its real job was resource-sharing between research computers, not surviving a war. Its own architects have spent decades correcting this popular myth.
                \item \textbf{TCP/IP} (Cerf \& Kahn, 1970s) connected ARPANET to other, different networks, with robustness against unreliable or damaged links as a real design goal.
            \end{itemize}
            \medskip
            \footnotesize Either way, the result is a decentralized network where packets route around damage --- a real property, whichever story motivated it.
        \end{column}
...
\begin{frame}{Closing activity (20 min): patterns across real sites}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            Pair up.
            \begin{enumerate}\small
                \item Pick two sites: one plain (a Wikipedia article, a blog) and one busy (a news homepage, a commercial site, a social feed).
                \item Explore both with the Inspector (10 min). Compare: \texttt{data-*} attributes, how deep the nesting gets, class names that look hand-written vs.\ auto-generated.
        ...
        \end{column}
        \begin{column}{0.35\textwidth}
            \begin{block}{Report back (7 min)}
                \footnotesize Round robin, \textasciitilde30 sec/pair:
                \begin{itemize}\footnotesize
                    \item One pattern from comparing your two sites
                    \item What you broke, and what happened
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
    \medskip
    \footnotesize Reminder: edits are local and temporary --- reload to undo, nothing reaches the server.
\end{frame}
