...
consistent. The finished **reference deck is `slides/week-06/slides.tex`** —
...
- `slides.tex` — the deck source (this spec)
- `slides.pdf` — compiled output (must build clean)
...
1. `\documentclass[9pt,aspectratio=169]{beamer}` then `\input{preamble}`
   (identical to week-06 — do **not** re-declare packages/theme/colors).
...
```bash
TEXINPUTS=../common: BIBINPUTS=../common: latexmk -pdf -interaction=nonstopmode -halt-on-error slides.tex
```

Then confirm all of:
- `slides.pdf` exists;
- `grep -ci undefined slides.log` returns `0`;
- page count ≥ 15 (`python3 -c "from pypdf import PdfReader; print(len(PdfReader('slides.pdf').pages))"`).
