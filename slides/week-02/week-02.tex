...
\begin{frame}{First: get this week's notebook from GitHub}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{enumerate}\small
                \item Go to \texttt{github.com/cuinfoscience/Web-Data-Science-Book}
                \item Open the \texttt{notebooks} folder, then click \texttt{ch-02-ethics.ipynb}
                \item Click \textbf{Download raw file} in the file toolbar (the down-arrow icon). If your browser opens the raw JSON instead of downloading it, right-click the button and choose \textbf{Save Link As\ldots}
                \item Save it into one folder you'll reuse all semester --- e.g.\ \texttt{\textasciitilde/INFO4617/} --- so you always know where this week's notebook landed
            \end{enumerate}
        \end{column}
        \begin{column}{0.35\textwidth}
            ...
            \end{block}
            \begin{alertblock}{One folder, all semester}
                \footnotesize Reusing the same folder means Jupyter always opens to a place with last week's notebook already in it too.
            \end{alertblock}
        \end{column}
    \end{columns}
    \bottomcite{Textbook Appendix A, ``Companion Notebooks.''}
\end{frame}

\begin{frame}[fragile]{Then: launch Jupyter and open it}
...
import os
print(os.getcwd())
# should print the folder you launched Jupyter from -- if it doesn't,
# stop the server (Ctrl+C), cd to the right folder, and relaunch
\end{lstlisting}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \footnotesize This three-command sequence --- \texttt{cd}, activate, launch --- is worth memorizing now. Every notebook lab this semester starts the same way.
        \end{column}
        \begin{column}{0.35\textwidth}
        ...
        \end{column}
    \end{columns}
    \bottomcite{\textit{Missing Manual} Ch.~16, ``Launching Jupyter the right way'' \& ``The common failure: `Jupyter shows no notebooks'.''}
\end{frame}
...
\end{lstlisting}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \footnotesize
            \textbf{Wikipedia} runs to about 700 lines and 460 \texttt{Disallow} rules, but it is permissive where it matters: \texttt{/wiki/} is open, and bulk users get the API and the dumps. \textbf{NY Times} restricts selectively; the content is a commercial product. \textbf{Reddit} is eight lines: \texttt{User-agent: *} then \texttt{Disallow: /}, plus a pointer to its Public Content Policy and a researcher program.
...
\begin{frame}{File it: your first issue}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{enumerate}
                \item Read the chapter: {\footnotesize \url{https://cuinfoscience.github.io/Web-Data-Science-Book/ch-02-ethics.html}}
