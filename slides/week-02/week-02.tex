...
rp = RobotFileParser()
rp.parse(text.splitlines())   # NOT rp.read() -- see the warning below
...
\begin{frame}[fragile]{The problem with \texttt{rp.read()}}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            The failure is \textbf{silent}. Look at the standard library:
...
\end{lstlisting}
            ...
            A 403 becomes ``this site disallows everything.'' No exception, no warning. Every \texttt{can\_fetch()} then returns \texttt{False}, and you conclude Wikipedia bans crawlers.
        \end{column}
        \begin{column}{0.35\textwidth}
            \begin{alertblock}{The chapter's own lesson}
                \footnotesize This is the failure the User-Agent rule exists to prevent. The library that reads the politeness file is itself impolite by default.

                \smallskip

                Fetch with \texttt{requests} and your header, then hand the text to \texttt{rp.parse()}.
            \end{alertblock}
        \end{column}
    \end{columns}
...
\end{lstlisting}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \footnotesize
            \textbf{Wikipedia} runs to about 700 lines and 460 \texttt{Disallow} rules, but it is permissive where it matters: \texttt{/wiki/} is open, and bulk users get the API and the dumps. \textbf{The New York Times} restricts selectively; the content is a commercial product. \textbf{Reddit} is eight lines: \texttt{User-agent: *} then \texttt{Disallow: /}, and a pointer to its Public Content Policy and a separate researcher program.

            \medskip
            \textbf{Line count is not restrictiveness.} Reddit's file is the shortest and the strictest of the three.
