...
\begin{frame}{How this book was written}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            This book was drafted using large language models $\rightarrow$ there is definitely important stuff that is wrong or confusing that needs to be fixed
...
\begin{frame}{Git \& GitHub}
    \begin{columns}[T]
        ...
        \end{column}
    \end{columns}
    \bottomcite{Setup instructions are in \texttt{handouts/week-01/setup.md} --- have this working before Monday.}
\end{frame}
...
\begin{frame}{Today's activity: file your first issue}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            \textbf{Goal:} by the end of class, every one of you has found and described one real issue with Chapter~1. If we run out of time, file it as homework --- have it posted before Monday.
...
\begin{frame}{Seven kinds of revision}
    ...
    \begin{columns}[T]
        ...
        \end{column}
    \end{columns}
    \bottomcite{The full framework, with worked examples of each type: \texttt{handouts/common/revision-framework.md}}
\end{frame}
...
\begin{frame}{Before Monday}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            Work through \texttt{handouts/week-01/setup.md} --- about 45 minutes:
            \begin{enumerate}
