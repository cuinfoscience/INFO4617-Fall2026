...
% Build:  latexmk -pdf week-14.tex   (from this folder; no env vars needed)
...
\begin{frame}{From exploration to production}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            The logic is the \textit{same code you already wrote}; only the environment changes. These are your existing skills, moved into production.
        \end{column}
        \begin{column}{0.35\textwidth}
            \centering
...
\begin{frame}{Automation: a snapshot vs.\ a time series}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
...
\begin{frame}[fragile]{Fail loudly: monitor \& harden}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            The dangerous failure is the \textbf{silent success}. The run reports no error, but the CSV row is empty. The site changed and the code raised no exception. Add a validation step to find this failure. Then stop the run cleanly:
\begin{lstlisting}
def main():
    try:
        save_scores(get_scores(URL))
    ...
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{enumerate}\small
                \item Build a scraper that runs in GitHub Actions. Deploy it. Run it for 3 or more days. Then analyze the data.
                \item Convert an earlier project into a \texttt{.py} script with no interface. Run the script from the terminal.
                \item Design a check that monitors the output. Use the file size, the row count, or the age of the data.
                \item Select a government source such as Open-Meteo or USGS. Collect data for one week. Make a visualization of the change.
                \item Write \texttt{check\_freshness.py}. The script must give a warning if the newest row is more than 24 hours old.
                \item[6.] \textbf{5617}: run a collector for 2 or more weeks. Then write a report about the infrastructure. Use \textcite{wilson2017good} as a model
            \end{enumerate}
        \end{column}
        \begin{column}{0.35\textwidth}
...
\begin{frame}{A good revision, and a good review}
    ...
    \end{columns}
    \medskip
    \centering\small Last deck of the semester --- Weeks 15--16 turn everything you've built into \textbf{research design} and your Final Project presentation (see the syllabus).
\end{frame}
