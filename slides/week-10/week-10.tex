...
\begin{frame}{This week at a glance}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \dayband{Friday}{Textbook revisions \textbar Proposal due}
            \begin{itemize}\small
...
\begin{frame}{What is an API?}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            An \textbf{API} (Application Programming Interface) is a \textbf{contract}. It specifies what you can request. It specifies which parameters you can send. It also specifies the format of the response.
            ...
            The tradeoff: an API only exposes what its operator \textit{chooses} to. You cannot request a field that the API does not include. You cannot exceed the rate limits. You cannot rely on the API forever. The ``post-API age'' later in this module lists many APIs that operators restricted, repriced, or shut down.
        \end{column}
        \begin{column}{0.35\textwidth}
            \centering
...
\begin{frame}{The case for Wikipedia}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
...
\begin{frame}{Lab: work these, then show-and-tell}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{enumerate}\small
                \item Compare the revision histories of two related articles. Report the unique editors, the edits for each editor, and the monthly activity.
                \item Get the daily pageviews of five articles. Report if the spikes occur together or separately.
                \item Find the outgoing wikilinks of three related articles. Report the links that they share.
                \item Read the API documentation. Write a request for a new endpoint such as \texttt{extlinks} or \texttt{recentchanges}.
                \item Find the 20 most frequent editors of an article. Then find the other articles that they edit.
                \item[6.] \textbf{5617}: reproduce one measure from \textcite{mesgari2015sum}. For example, calculate a Gini coefficient of the edits for each editor
            \end{enumerate}
        \end{column}
        \begin{column}{0.35\textwidth}
