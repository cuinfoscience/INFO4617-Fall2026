...
\begin{frame}{Today's case study --- link the Missing Manual}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            Open \texttt{ch-04-data-formats.qmd} and find this line:

            \medskip
            \textit{``For more on data file formats\ldots{} see Missing Manual Chapter 20: Data File Formats.''}

            \medskip
            It names a chapter. It does not link to it. A reader has to leave the page, find the Missing Manual site, and search for Chapter 20 by hand.

            \medskip
            Your fix: wrap that name in a Markdown link. Eight steps, start to finish.
        \end{column}
        \begin{column}{0.35\textwidth}
...
\begin{frame}{What happens next}
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \medskip
            When it merges, the book republishes itself. Reload the chapter in a few minutes --- your link is live.
        \end{column}
        \begin{column}{0.35\textwidth}
